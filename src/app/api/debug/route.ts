...
const getUsernameByRank = async (rank: number): Promise<any> => {
  try {
    const pageIndex = Math.ceil(rank / usersPerPage);
    ...
    const userData = getUserInfo(responseData, rank);
    return userData;
  } catch (error: any) {
    console.error(`Failed to Fetch User Details by rank : ${rank}`);
    throw error;
  }
};
...
export async function GET() {
  try {
    ...
    const data = await getUsernameByRank(9941);
